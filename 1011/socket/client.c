...
int main(int argv, char *argc[])
{
    ...
    server_addr.sin_addr.s_addr = inet_addr(argc[1]); // 서버 IP 주소 설정
    server_addr.sin_port = htons(atoi(argc[2]));      // 서버 포트 번호 설정 (문자열을 정수로 변환 후 네트워크 바이트 순서로 변환)
